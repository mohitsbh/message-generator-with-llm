...
exports.handler = async function (event, context) {
  ...
  const { prompt, useLLM, provider } = body || {};
  if (typeof prompt !== 'string') return { statusCode: 400, body: JSON.stringify({ error: 'prompt string required' }) };
  ...
  return { statusCode: 200, body: JSON.stringify({ message: fallback }) };
};
